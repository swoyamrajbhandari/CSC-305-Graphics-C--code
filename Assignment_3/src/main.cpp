...
double ray_parallelogram_intersection(const Vector3d &ray_origin, const Vector3d &ray_direction, int index, Vector3d &p, Vector3d &N)
{
    ...
    Matrix3d M;
    M << -pgram_u, -pgram_v, ray_direction;

    Vector3d t = M.colPivHouseholderQr().solve(pgram_origin - ray_origin);

    if (t[2] < 0 || t[0] < 0 || t[0] > 1 || t[1] < 0 || t[1] > 1)
    {
        return -1;
    }

    p = ray_origin + t[2] * ray_direction;
...
Vector4d shoot_ray(const Vector3d &ray_origin, const Vector3d &ray_direction, int max_bounce)
{
    ...
    for (int i = 0; i < light_positions.size(); ++i)
    {
        ...
        if (is_light_visible(p + N * 1e-4, Li, light_position)) {
    ...
    if (max_bounce != 0)
    {
        ...
        const Vector3d offset_ray_origin = p + 0.0001 * r;

        reflection_color = (refl_color.cwiseProduct(shoot_ray(offset_ray_origin, r, max_bounce - 1)));
    }
